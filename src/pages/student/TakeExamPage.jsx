...
const TakeExamPage = () => {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      const response = await examsService.submitExamAttempt(examId, answers);
  ...
  const handleConfirmSubmit = () => {
    if (window.confirm('هل أنت متأكد من تقديم الاختبار؟ لا يمكنك العودة بعد التقديم.')) {
      handleSubmit();
    }
  };
  ...
  // ── Results View ─────────────────────────────────────────────────
  if (result) {
    return <ResultView result={result} examData={examData} darkMode={darkMode} navigate={navigate} />;
  }
      ...
      <Box sx={{
        ...
        <Container maxWidth="xl">
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 1 }}>
            <IconButton
              onClick={() => {
                if (window.confirm('هل تريد الخروج من الاختبار؟ سيتم فقدان التقدم.')) {
        ...
        <Box sx={{ display: 'flex', gap: 3, flexDirection: { xs: 'column', md: 'row' } }}>
          ...
          <Box sx={{ flex: 1 }}>
            {questions.map((q, i) => {
              ...
              const isMCQorTF = ['multiple-choice', 'true-false', 'true/false', 'mcq'].includes(qType);
                    ...
                    <CardContent sx={{ p: 3 }}>
                      ...
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                        ...
                        <Box sx={{ display: 'flex', gap: 1 }}>
                          <Chip
                            label={q.type === 'true-false' || q.type === 'true/false' ? 'صح/خطأ'
                              : q.type === 'essay' ? 'مقالي'
                              : 'اختيار من متعدد'}
                            size="small"
                            variant="outlined"
                                ...
                                >
                                  <CardContent sx={{ py: 1.5, px: 2, '&:last-child': { pb: 1.5 } }}>
                                    <FormControlLabel
                                      ...
                                      label={
                                        <Typography fontFamily="Cairo, sans-serif" sx={{ color: darkMode ? '#e2e8f0' : '#334155' }}>
                                          {option}
                                        </Typography>
                                      }
                                      sx={{ width: '100%', m: 0 }}
        ...
        {hasEssays && (
          ...
          >
            ...
            <Typography variant="body2" fontFamily="Cairo, sans-serif">
              تم تصحيح {result.autoTotal} درجة من الأسئلة التلقائية. 
              الأسئلة المقالية ({result.essayCount}) تحتاج إلى مراجعة من المدرس.
              ستظهر النتيجة النهائية بعد التصحيح.
            </Typography>
          </Alert>
        )}
            ...
            <CardContent sx={{ p: 3 }}>
              ...
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
                {result.gradedDetails.map((detail, i) => {
                  const qText = examData?.questions?.find(q => q.id === detail.questionId)?.questionText || `سؤال ${i + 1}`;
                    ...
                    <Box key={detail.questionId} sx={{
                      ...
                      <Box sx={{ flex: 1, minWidth: 0 }}>
                        <Typography variant="body2" fontWeight={600} fontFamily="Cairo, sans-serif"
                          ...
                          {qText.length > 60 ? qText.substring(0, 60) + '...' : qText}
                        </Typography>
                      ...
                      <Typography variant="body2" fontWeight={700}
                        sx={{ color: isEssayPending ? '#2563eb' : isCorrect ? '#059669' : '#ef4444' }}>
                        {isEssayPending ? '—' : `${detail.marks}/${detail.maxMarks}`}
                      </Typography>
                    </Box>
                  );
                })}
