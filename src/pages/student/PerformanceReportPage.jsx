...
// ✅ استيراد البيانات من ملف studentData
import {
  studentData,
  performanceStatsData,
  rankingData,
  subjectsPerformanceData,
  courseProgressStatsData,
  weeklyProgressData,
...
// تكوين الأيقونات والألوان للإحصائيات
const STATS_CONFIG = {
...
const PerformanceReportPage = () => {
  ...
  // تحويل البيانات للإحصائيات مع الأيقونات
  const stats = performanceStatsData.map((stat) => ({
    ...stat,
    icon: STATS_CONFIG[stat.type]?.icon || StarIcon,
    color: STATS_CONFIG[stat.type]?.color || '#64748b',
  }));

  // بيانات مقارنة الدرجات
  const gradesChartData = {
    labels: subjectsPerformanceData.map((s) => s.name),
    datasets: [
      {
        label: 'درجاتك',
        data: subjectsPerformanceData.map((s) => s.grade),
        backgroundColor: 'rgba(37, 99, 235, 0.8)',
        borderRadius: 8,
      },
      {
        label: 'متوسط الفصل',
        data: subjectsPerformanceData.map((s) => s.classAvg),
  ...
  // بيانات تقدم الأداء عبر الوقت
  const progressChartData = {
    labels: weeklyProgressData.labels,
    datasets: [
      {
        label: 'الدرجات',
        data: weeklyProgressData.studentGrades,
        borderColor: '#2563eb',
      ...
      {
        label: 'متوسط الفصل',
        data: weeklyProgressData.classAverage,
        borderColor: '#94a3b8',
  ...
  // بيانات تقدم الكورسات
  const courseProgressChartData = {
    labels: ['مكتمل', 'قيد التقدم', 'لم يبدأ'],
    datasets: [
      {
        data: [
          courseProgressStatsData.completed,
          courseProgressStatsData.inProgress,
          courseProgressStatsData.notStarted,
        ],
  ...
  // حساب التقدم في المراكز
  const rankImprovement = rankingData.previousRank - rankingData.classRank;
            ...
            >
              ...
              <Box sx={{ textAlign: 'center', mb: 3 }}>
                ...
                {rankImprovement > 0 && (
                  <Chip
                    icon={<TrendingUpIcon sx={{ fontSize: 14 }} />}
                    label={`تقدمت ${rankImprovement} مراكز`}
                    size="small"
                    sx={{
                      mt: 1,
                      bgcolor: '#dcfce7',
...
<Grid item xs={12}>
  ...
  >
    ...
    <Grid container spacing={2}>
      {subjectsPerformanceData.map((subject) => (
        <Grid item xs={12} sm={6} md={4} lg={2.4} key={subject.id}>
          <Box
            sx={{
              p: 2,
              borderRadius: 2,
