...
// Purpose: Registration callback with single subject selection for teachers and username generation
...
const ClerkRegisterCallbackPage = () => {
  ...
  useEffect(() => {
    const checkUserStatus = async () => {
      try {
        ...
        console.log('🔍 Existing metadata:', { existingRole, existingDivision, existingSubject, existingUsername });
        ...
        if (existingRole) {
          ...
          setTimeout(async () => {
            ...
            await handleLogin(clerkUser.id, existingRole, existingDivision, existingSubject, existingUsername);
          }, 3000);
  ...
  const handleLogin = async (clerkUserId, role = 'student', division = null, subject = null, username = null) => {
    if (isProcessing) {
    ...
    try {
      console.log('🔐 Starting login process...', { clerkUserId, role, division, subject, username });
      ...
      await loginWithClerk(clerkUserId, role, division, subject, username);
  ...
  const handleRoleSelect = (role) => {
    ...
    // Generate username if applicable
    if (shouldGenerateUsername(role)) {
        ...
        <Box sx={{ width: '100%', py: 4 }}>
          ...
          >
            ...
            <Typography sx={{ fontFamily: 'Cairo, sans-serif', fontWeight: 600, color: 'text.secondary' }}>
              سيتم تحويلك لتسجيل الدخول تلقائياً بعد 3 ثواني...
            </Typography>
          </Alert>
      ...
      <Container maxWidth="sm" sx={{ minHeight: '100vh', display: 'flex', alignItems: 'center' }}>
        <Box sx={{ width: '100%', py: 4 }}>
          <Box sx={{ textAlign: 'center', mb: 4 }}>
            ...
            <Typography variant="body2" fontWeight={700} fontFamily="Cairo, sans-serif" sx={{ color: '#7c3aed' }}>
