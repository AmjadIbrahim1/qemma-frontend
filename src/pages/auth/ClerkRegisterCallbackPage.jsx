...
const ClerkRegisterCallbackPage = () => {
  ...
  const handleSubjectConfirm = () => {
    ...
    else handleLogin(clerkUser.id, selectedRole, null, selectedSubject, null, null, null, derivedStream, phone);
  };
  ...
  const handleVerifyTeacherCode = async () => {
    ...
    try {
      ...
      const derivedStream = getStreamFromSubject(selectedSubject);
      handleLogin(clerkUser.id, 'assistant_teacher', null, selectedSubject, teacherUsername.trim(), null, null, derivedStream, phone);
    } catch (err) {
      ...
      <Container maxWidth="sm" sx={{ minHeight: '100vh', display: 'flex', alignItems: 'center' }}>
        <Box sx={{ width: '100%', py: 4 }}>
          ...
          <FormControl fullWidth error={!!yearError} sx={{ mb: 3 }}>
            ...
            <Select value={selectedYear} onChange={(e) => { setSelectedYear(e.target.value); setYearError(''); }}
