...
// ✅ FIX: teacherName يُمرر عند doRegister لضمان ربط المدرس المساعد بالمدرس الرئيسي
...
import {
  ...
  InputAdornment, IconButton, CircularProgress, Card, CardContent,
  FormControl, InputLabel, Select, MenuItem, FormHelperText, Alert, Chip,
} from '@mui/material';
import {
  Visibility, VisibilityOff, Lock, Person, Phone, ArrowBack,
  School, Person as PersonIcon, FamilyRestroom, SupervisorAccount,
  Send, CheckCircle, Search, AlternateEmail,
...
const RegisterPage = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    role:            'student',
    division:        '',
    year:            '',   // ✅ NEW: student academic year (StudentYear enum: first/second/third)
    subject:         '',
  });
  ...
  const validateForm = () => {
    ...
    // ✅ NEW: year is required for students (used by contest-reminder notification filter)
    if (formData.role === 'student' && !formData.year)
      newErrors.year = 'يرجى اختيار الصف الدراسي';
    if (['teacher', 'assistant_teacher'].includes(formData.role) && !formData.subject)
  ...
  const handleSubmit = async (e) => {
    ...
    // Check phone uniqueness before navigating to verification
    if (formData.phone && ['assistant_teacher', 'parent'].includes(formData.role)) {
      try {
    ...
    if (formData.role === 'assistant_teacher') { setStep('verify-assistant'); return; }
    if (formData.role === 'parent')            { setStep('verify-parent');    return; }
    await doRegister();
  };

  // ✅ FIX: قبول teacherName كمعامل اختياري وتمريره للـ backend
  const doRegister = async (teacherName = null, studentUsername = null) => {
    ...
    try {
      ...
      // strip empty phone so backend .optional() skips validation
      if (!registerData.phone) delete registerData.phone;

      if (registerData.role !== 'student')                              delete registerData.division;
      if (!['teacher','assistant_teacher'].includes(registerData.role)) delete registerData.subject;

      // ✅ NEW: year is student-only — strip for non-students (avoid empty-string failing backend .optional() validation)
      if (registerData.role !== 'student') delete registerData.year;

      // ✅ NEW: teacher stream is derived from subject (user does NOT choose stream manually).
      // Backend also derives as a fallback; sending it here keeps frontend/backend in sync.
      if (['teacher','assistant_teacher'].includes(registerData.role)) {
        ...
        else delete registerData.stream;   // no mapping → omit (backend .optional() would reject '')
      } else {
        delete registerData.stream;
      }

      // ✅ تمرير teacherName فقط للمدرس المساعد
      ...
      // ✅ تمرير studentUsername لربط ولي الأمر بالطالب
      ...
      const dashboardRoute =
        registerData.role === 'student'             ? '/student/dashboard'
        : registerData.role === 'teacher'           ? '/teacher/dashboard'
        : registerData.role === 'assistant_teacher' ? '/assistant-teacher/dashboard'
        : registerData.role === 'parent'            ? '/parent/dashboard'
        : '/student/dashboard';
...
const phoneError = errors.find(
  e => e.param === 'phone' || e.path === 'phone'
    ...
    if (phoneError) {
      ...
      if (['assistant_teacher', 'parent'].includes(registerData.role)) {
        setStep('local');
      }
  ...
  const handleLookupTeacher = async () => {
    if (!teacherUsernameInput.trim()) { setVerificationError('يرجى إدخال اسم المستخدم للمدرس'); return; }
    setLookupLoading(true); setVerificationError(''); setTeacherInfo(null);
    try {
      const res = await API.get(`/auth/assistant/teacher/${teacherUsernameInput.trim()}`);
      setTeacherInfo(res.data.data);
    } catch (err) {
      setVerificationError(err.response?.data?.message || 'لم يتم العثور على مدرس بهذا الاسم المميز');
  ...
  const handleSendCodeToTeacher = async () => {
    ...
    try {
      await API.post('/auth/assistant/send-code', { teacherUsername: teacherUsernameInput.trim(), assistantEmail: fullEmail });
      setCodeSent(true);
  ...
  // ✅ FIX: تمرير teacherUsernameInput لـ doRegister بعد نجاح التحقق
  const handleVerifyTeacherCode = async () => {
    if (!verificationCode.trim() || verificationCode.length !== 6) { setVerificationError('يرجى إدخال الكود المكون من 6 أرقام'); return; }
    setVerifyLoading(true); setVerificationError('');
    try {
      await API.post('/auth/assistant/verify-code', { teacherUsername: teacherUsernameInput.trim(), code: verificationCode.trim() });
      toast.success('تم التحقق بنجاح! ✅');
      // ✅ تمرير username المدرس لربط المدرس المساعد به في قاعدة البيانات
  ...
  const handleLookupStudent = async () => {
    if (!studentUsernameInput.trim()) { setParentVerificationError('يرجى إدخال اسم المستخدم للطالب'); return; }
    setParentLookupLoading(true); setParentVerificationError(''); setStudentInfo(null);
    try {
      const res = await API.get(`/auth/parent/student/${studentUsernameInput.trim()}`);
      setStudentInfo(res.data.data);
    } catch (err) {
      setParentVerificationError(err.response?.data?.message || 'لم يتم العثور على طالب بهذا الاسم المميز');
  ...
  const handleSendCodeToStudent = async () => {
    ...
    try {
      await API.post('/auth/parent/send-code', { studentUsername: studentUsernameInput.trim(), parentEmail: fullEmail });
  ...
  const handleVerifyParentCode = async () => {
    if (!parentCode.trim() || parentCode.length !== 6) { setParentVerificationError('يرجى إدخال الكود المكون من 6 أرقام'); return; }
    setParentVerifyLoading(true); setParentVerificationError('');
    try {
      await API.post('/auth/parent/verify-code', { studentUsername: studentUsernameInput.trim(), code: parentCode.trim() });
      toast.success('تم التحقق بنجاح! ✅');
      // ✅ Pass studentUsername so backend links parent to student
  ...
  const handleOpenClerkModal = async () => {
    ...
    } catch (_) { setShowClerkModal(true); }
  };
      ...
      <Container maxWidth="sm" sx={{ minHeight: '100vh', display: 'flex', alignItems: 'center' }}>
        <Box sx={{ width: '100%', py: 4 }}>
          ...
          <Box sx={{ textAlign: 'center', mb: 4 }}>
            ...
            <Typography variant="h5" fontWeight={700} fontFamily="Cairo, sans-serif" sx={{ mt: 1 }}>إنشاء حساب جديد</Typography>
            <Typography variant="body2" color="text.secondary" fontFamily="Cairo, sans-serif">اختر طريقة التسجيل المناسبة لك</Typography>
          </Box>
          ...
          <Button fullWidth variant="outlined" size="large" onClick={handleOpenClerkModal}
            sx={{ mb: 2, fontFamily: 'Cairo, sans-serif', fontWeight: 700, borderRadius: 2, py: 1.5, borderColor: '#E5E7EB', color: 'text.primary' }}
            startIcon={
          ...
          <Box sx={{ mt: 3, textAlign: 'center' }}>
            <Typography variant="body2" fontFamily="Cairo, sans-serif">
              لديك حساب بالفعل؟{' '}
              <Link to="/login" state={redirectTo ? { redirectTo, redirectState, message: redirectMsg } : undefined}
          ...
          {showClerkModal && (
            <Box sx={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, bgcolor: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999 }}
              ...
              <Box onClick={(e) => e.stopPropagation()}>
                <SignUp routing="virtual" afterSignUpUrl="/auth/clerk-register-callback" />
              </Box>
            </Box>
          )}
        </Box>
  ...
  if (step === 'verify-assistant') {
    return (
      <Container maxWidth="sm" sx={{ minHeight: '100vh', display: 'flex', alignItems: 'center' }}>
        <Box sx={{ width: '100%', py: 4 }}>
          <Button startIcon={<ArrowBack />} onClick={() => setStep('local')} sx={{ mb: 3, fontFamily: 'Cairo, sans-serif', fontWeight: 700 }}>رجوع</Button>
          <Box sx={{ textAlign: 'center', mb: 4 }}>
            <Typography variant="h4" fontWeight={900} fontFamily="Cairo, sans-serif" sx={{ mb: 1 }}>ربط حساب المدرس المساعد 🔗</Typography>
            <Typography variant="body2" color="text.secondary" fontFamily="Cairo, sans-serif">أدخل اسم المستخدم الخاص بالمدرس الذي ستساعده</Typography>
          </Box>

          <Box sx={{ mb: 3 }}>
            <Typography variant="subtitle2" fontFamily="Cairo, sans-serif" fontWeight={700} sx={{ mb: 1 }}>الخطوة 1: ابحث عن المدرس بالاسم المميز</Typography>
            <Box sx={{ display: 'flex', gap: 1 }}>
              <TextField fullWidth placeholder="مثال: tch_SwiftEagle42" value={teacherUsernameInput}
                onChange={(e) => { setTeacherUsernameInput(e.target.value); setVerificationError(''); }}
            ...
            <Card elevation={0} sx={{ mb: 3, border: '2px solid #7c3aed', borderRadius: 2 }}>
              <CardContent sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                ...
                <Box sx={{ flex: 1 }}>
                  ...
                  <Box sx={{ mt: 0.5 }}><Chip label="مدرس" size="small" sx={{ bgcolor: '#f5f3ff', color: '#7c3aed', fontFamily: 'Cairo, sans-serif', fontWeight: 700 }} /></Box>
                </Box>
          ...
          {teacherInfo && !codeSent && (
            <Box sx={{ mb: 3 }}>
              ...
              <Alert severity="info" sx={{ mb: 2, fontFamily: 'Cairo, sans-serif' }}>
                سيصل للمدرس <strong>{teacherInfo.name}</strong> كود مكون من 6 أرقام في الوقت الفعلي عبر الإشعارات.
              </Alert>
          ...
          {codeSent && (
            <Box sx={{ mb: 3 }}>
              <Typography variant="subtitle2" fontFamily="Cairo, sans-serif" fontWeight={700} sx={{ mb: 1 }}>الخطوة 3: أدخل الكود الذي أرسله لك المدرس</Typography>
      ...
      <Container maxWidth="sm" sx={{ minHeight: '100vh', display: 'flex', alignItems: 'center' }}>
        <Box sx={{ width: '100%', py: 4 }}>
          ...
          <Box sx={{ textAlign: 'center', mb: 4 }}>
            ...
            <Typography variant="body2" color="text.secondary" fontFamily="Cairo, sans-serif">أدخل اسم المستخدم الخاص بابنك/ابنتك</Typography>
          </Box>

          <Box sx={{ mb: 3 }}>
            <Typography variant="subtitle2" fontFamily="Cairo, sans-serif" fontWeight={700} sx={{ mb: 1 }}>الخطوة 1: ابحث عن الطالب بالاسم المميز</Typography>
            <Box sx={{ display: 'flex', gap: 1 }}>
              <TextField fullWidth placeholder="مثال: std_BraveWolf42" value={studentUsernameInput}
                onChange={(e) => { setStudentUsernameInput(e.target.value); setParentVerificationError(''); }}
                InputProps={{ startAdornment: <InputAdornment position="start"><Search /></InputAdornment> }}
          ...
          {studentInfo && (
            <Card elevation={0} sx={{ mb: 3, border: '2px solid #db2777', borderRadius: 2 }}>
              <CardContent sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                <Box sx={{ width: 48, height: 48, borderRadius: '50%', bgcolor: '#fdf2f8', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                ...
                <Box sx={{ flex: 1 }}>
                  ...
                  <Box sx={{ mt: 0.5 }}><Chip label="طالب" size="small" sx={{ bgcolor: '#fdf2f8', color: '#db2777', fontFamily: 'Cairo, sans-serif', fontWeight: 700 }} /></Box>
                </Box>
                <CheckCircle sx={{ color: '#059669' }} />
              </CardContent>
            </Card>
          ...
          {parentCodeSent && (
            <Box sx={{ mb: 3 }}>
              <Typography variant="subtitle2" fontFamily="Cairo, sans-serif" fontWeight={700} sx={{ mb: 1 }}>الخطوة 3: أدخل الكود الذي أرسله لك الطالب</Typography>
  ...
  return (
    <Container maxWidth="sm" sx={{ minHeight: '100vh', display: 'flex', alignItems: 'center' }}>
      <Box sx={{ width: '100%', py: 4 }}>
        <Button startIcon={<ArrowBack />} onClick={() => setStep('choice')} sx={{ mb: 3, fontFamily: 'Cairo, sans-serif', fontWeight: 700 }}>العودة</Button>
        ...
        <Box sx={{ textAlign: 'center', mb: 4 }}>
          ...
          <Typography variant="h5" fontWeight={700} fontFamily="Cairo, sans-serif" sx={{ mt: 1 }}>إنشاء حساب جديد</Typography>
          <Typography variant="body2" color="text.secondary" fontFamily="Cairo, sans-serif">أكمل البيانات التالية للتسجيل</Typography>
        </Box>
        ...
        <Box component="form" onSubmit={handleSubmit} noValidate>
          {/* Role selector */}
          <Box sx={{ mb: 3 }}>
            <Typography variant="subtitle2" fontFamily="Cairo, sans-serif" fontWeight={700} sx={{ mb: 2 }}>نوع الحساب</Typography>
            <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 1.5 }}>
              {[
                ...
                <Card key={roleOption.value}
                  onClick={() => setFormData((prev) => ({ ...prev, role: roleOption.value }))}
                  sx={{
                    cursor: 'pointer', border: '2px solid',
                    borderColor: formData.role === roleOption.value ? getRoleColor(roleOption.value) : 'transparent',
                    bgcolor: formData.role === roleOption.value ? `${getRoleColor(roleOption.value)}10` : 'background.paper',
                    transition: 'all 0.2s',
                    '&:hover': { borderColor: getRoleColor(roleOption.value), transform: 'translateY(-2px)' },
                  }}>
                  <CardContent sx={{ textAlign: 'center', py: 2, px: 1 }}>
                    <Box sx={{ color: getRoleColor(roleOption.value), mb: 0.5 }}>{roleOption.icon}</Box>
                    <Typography variant="body2" fontFamily="Cairo, sans-serif" fontWeight={700}>{roleOption.label}</Typography>
                  </CardContent>
                </Card>
              ))}
            </Box>
          ...
          <Alert severity="info" sx={{ mb: 2, fontFamily: 'Cairo, sans-serif', fontSize: '0.85rem' }}>
          ...
          {formData.role === 'student' && (
            <FormControl fullWidth sx={{ mb: 2 }} error={!!errors.division}>
              ...
              <Select name="division" value={formData.division} onChange={handleChange} label="القسم الدراسي *" sx={{ fontFamily: 'Cairo, sans-serif' }}>
                {/* OLD stream values — migrated to canonical names per planning_prompts decision 3
                ...
                */}
                {/* ✅ NEW canonical stream values (Literary/Science-Maths/Science-Biology) — match Contest.stream/Teacher.stream */}
                <MenuItem value="Science-Maths">علمي رياضة</MenuItem>
                <MenuItem value="Science-Biology">علمي علوم</MenuItem>
                <MenuItem value="Literary">أدبي</MenuItem>
              </Select>
              {errors.division && <FormHelperText>{errors.division}</FormHelperText>}
            </FormControl>
          )}

          {/* ✅ NEW: student academic year select (StudentYear enum: first/second/third) */}
          {formData.role === 'student' && (
            <FormControl fullWidth sx={{ mb: 2 }} error={!!errors.year}>
              <InputLabel sx={{ fontFamily: 'Cairo, sans-serif' }}>الصف الدراسي *</InputLabel>
              <Select name="year" value={formData.year} onChange={handleChange} label="الصف الدراسي *" sx={{ fontFamily: 'Cairo, sans-serif' }}>
                {YEAR_OPTIONS.map((y) => (
          ...
          {isTeacherRole && (
            ...
            <FormControl fullWidth sx={{ mb: 2 }} error={!!errors.subject}>
              ...
              <Select name="subject" value={formData.subject} onChange={handleChange} label="المادة الدراسية *" sx={{ fontFamily: 'Cairo, sans-serif' }}>
              ...
              {errors.subject ? <FormHelperText>{errors.subject}</FormHelperText>
                : <FormHelperText sx={{ fontFamily: 'Cairo, sans-serif' }}>اختر المادة التي ستقوم بتدريسها</FormHelperText>}
            </FormControl>
            ...
            {formData.subject && getStreamFromSubject(formData.subject) && (
              <Alert severity="info" sx={{ mb: 2, fontFamily: 'Cairo, sans-serif', fontSize: '0.85rem' }}>
                القسم المُستنتج من المادة: <strong>{STREAM_LABELS[getStreamFromSubject(formData.subject)]}</strong>
              </Alert>
            )}
            </>
          )}
          ...
          {formData.role === 'assistant_teacher' && (
            <Alert severity="info" sx={{ mb: 2, fontFamily: 'Cairo, sans-serif', fontSize: '0.85rem' }}>⚡ بعد ملء البيانات ستحتاج للتحقق من هوية المدرس عبر كود OTP</Alert>
          )}
          {formData.role === 'parent' && (
            <Alert severity="info" sx={{ mb: 2, fontFamily: 'Cairo, sans-serif', fontSize: '0.85rem' }}>⚡ بعد ملء البيانات ستحتاج للتحقق عبر الطالب عبر كود OTP</Alert>
          )}

          <TextField fullWidth name="name" label="الاسم الكامل" value={formData.name}
            onChange={handleChange} error={!!errors.name} helperText={errors.name}
            InputProps={{ startAdornment: <InputAdornment position="start"><Person /></InputAdornment> }}
            sx={{ mb: 2 }} />

          <TextField
            fullWidth label="البريد الإلكتروني" value={emailPrefix}
            onChange={handleEmailPrefixChange} error={!!errors.email}
            helperText={errors.email || (emailPrefix ? `سيكون بريدك: ${fullEmail}` : `يجب أن ينتهي بـ ${DOMAIN}`)}
            placeholder="اسمك" dir="ltr"
            InputProps={{
              startAdornment: <InputAdornment position="start"><AlternateEmail /></InputAdornment>,
              endAdornment: (
                <InputAdornment position="end">
                  <Box sx={{ bgcolor: '#f1f5f9', px: 1.5, py: 0.5, borderRadius: 1, fontFamily: 'Cairo, sans-serif', fontSize: '0.85rem', fontWeight: 700, color: '#475569', whiteSpace: 'nowrap' }}>
                    {DOMAIN}
                  </Box>
                </InputAdornment>
              ),
          ...
          <TextField fullWidth name="phone" label="رقم الهاتف *" placeholder="01012345678"
            value={formData.phone} onChange={handleChange} error={!!errors.phone} helperText={errors.phone}
            InputProps={{ startAdornment: <InputAdornment position="start"><Phone /></InputAdornment> }}
            sx={{ mb: 2 }} />

          <TextField fullWidth name="password" label="كلمة المرور" type={showPassword ? 'text' : 'password'}
          ...
          <TextField fullWidth name="confirmPassword" label="تأكيد كلمة المرور" type={showConfirmPassword ? 'text' : 'password'}
          ...
          <Button fullWidth type="submit" variant="contained" size="large" disabled={loading} disableElevation
            sx={{ fontFamily: 'Cairo, sans-serif', fontWeight: 900, borderRadius: 2, py: 1.5, background: 'linear-gradient(135deg, #2563eb 0%, #7c3aed 55%, #db2777 100%)' }}>
            {loading ? <CircularProgress size={24} color="inherit" /> : (
              formData.role === 'assistant_teacher' ? 'التالي: التحقق من المدرس →' :
              formData.role === 'parent'            ? 'التالي: التحقق من الطالب →' :
              'إنشاء حساب'
            )}
          </Button>
        </Box>
