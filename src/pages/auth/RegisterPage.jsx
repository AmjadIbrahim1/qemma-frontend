...
const RegisterPage = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    subject: "",
    username: "",
    teacherName: "", // For assistant_teacher
  });
  ...
  // Generate username when role changes (only for students and teachers, NOT assistant_teacher)
  useEffect(() => {
  ...
  const validateForm = () => {
    ...
    // Validate teacherName for assistant_teacher
    if (formData.role === 'assistant_teacher' && !formData.teacherName.trim()) {
      newErrors.teacherName = "يجب إدخال اسم المدرس";
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Only include username for students and teachers
      if (registerData.role === 'assistant_teacher' || registerData.role === 'parent') {
        delete registerData.username;
      }

      // Only include teacherName for assistant_teacher
  ...
  const shouldShowUsername = formData.role === 'student' || formData.role === 'teacher';
    ...
    >
      <Box sx={{ width: "100%", py: 4 }}>
        ...
        <Box component="form" onSubmit={handleSubmit} noValidate>
          ...
          {/* Username Display - Only for students and teachers */}
          ...
          {/* Subject selection - Only for teachers and assistant teachers */}
          {isTeacherRole && (
          ...
          {formData.role === 'assistant_teacher' && (
            <TextField
              ...
              helperText={errors.teacherName || "أدخل اسم المدرس الذي تساعده"}
