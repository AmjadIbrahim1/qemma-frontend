  ...
  return (
    <Box sx={{ minHeight: '100vh', bgcolor: darkMode ? '#0f172a' : '#f9fafb', py: 4 }}>
      <Container maxWidth="md">
        ...
        <Box sx={{ mb: 4 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 2 }}>
            ...
            <Typography variant="h4" fontWeight={900} fontFamily="Cairo, sans-serif" sx={{ color: darkMode ? '#f1f5f9' : 'inherit' }}>
              {editingId ? 'تعديل حصة' : 'إضافة حصة جديدة'}
            </Typography>
          </Box>
        ...
        {/* Form Card */}
        <Card elevation={0} sx={{ border: '1px solid', borderColor: darkMode ? '#334155' : '#e5e7eb', bgcolor: darkMode ? '#1e293b' : 'white', mb: 4 }}>
          <CardContent sx={{ p: 4 }}>
            <form onSubmit={handleSubmit}>
              <Grid container spacing={3}>
                ...
                <Grid item xs={12}>
                  <TextField
                    fullWidth required name="title" label="عنوان الحصة"
                    value={formData.title} onChange={handleInputChange}
                    placeholder="مثال: مراجعة الوحدة الأولى"
                    InputProps={{ startAdornment: <Class sx={{ mr: 1, color: '#2563eb' }} /> }}
                    sx={{
                      '& .MuiOutlinedInput-root': { fontFamily: 'Cairo, sans-serif', bgcolor: darkMode ? '#0f172a' : '#f9fafb' },
                      '& .MuiInputLabel-root': { fontFamily: 'Cairo, sans-serif', fontWeight: 600 },
                    }}
                  />
                </Grid>
                ...
                <Grid item xs={12}>
                  <TextField
                    fullWidth select name="course" label="اختر الكورس"
                  ...
                  >
                    ...
                    {courses.map(course => (
                      <MenuItem key={course.id} value={course.id} sx={{ fontFamily: 'Cairo, sans-serif' }}>
                        {course.title}
                      </MenuItem>
                    ))}
                  </TextField>
                ...
                <Grid item xs={12} md={6}>
                  <TextField
                    fullWidth type="date" name="date" label="التاريخ"
                ...
                <Grid item xs={12} md={6}>
                  <TextField
                    fullWidth select name="type" label="نوع الحصة"
                ...
                {/* وقت البداية — الوقت الحالي تلقائياً */}
                <Grid item xs={12} md={6}>
                  <TextField
                    fullWidth type="time" name="startTime" label="وقت البداية"
                ...
                <Grid item xs={12} md={6}>
                  <TextField
                    fullWidth type="time" name="endTime" label="وقت النهاية"
                    value={formData.endTime} onChange={handleInputChange}
                    required
                    InputLabelProps={{ shrink: true }}
                    InputProps={{ startAdornment: <Schedule sx={{ mr: 1, color: '#f59e0b' }} /> }}
                    helperText="ساعة من الآن تلقائياً"
                    sx={{
                      '& .MuiOutlinedInput-root': { fontFamily: 'Cairo, sans-serif', bgcolor: darkMode ? '#0f172a' : '#f9fafb' },
                      '& .MuiInputLabel-root': { fontFamily: 'Cairo, sans-serif', fontWeight: 600 },
                    }}
                ...
                {formData.type === 'online' && (
                  <Grid item xs={12}>
                    <TextField
                      fullWidth name="meetingLink" label="رابط الحصة (Zoom, Google Meet, etc.)"
                ...
                <Grid item xs={12} md={6}>
                  <TextField
                    fullWidth type="number" name="maxStudents" label="الحد الأقصى للطلاب (اختياري)"
                ...
                <Grid item xs={12}>
                  <TextField
                    fullWidth multiline rows={4} name="description" label="وصف الحصة (اختياري)"
                  ...
                  <Box sx={{ display: 'flex', gap: 2, justifyContent: 'flex-end' }}>
                    ...
                    <Button
                      ...
                      sx={{
                        ...
                        fontFamily: 'Cairo, sans-serif', fontWeight: 700, px: 4,
                        '&:hover': { background: 'linear-gradient(135deg, #1d4ed8 0%, #6d28d9 100%)' },
                        '&:disabled': { opacity: 0.7 },
                      }}
                    >
                      {submitting ? 'جارٍ الحفظ...' : editingId ? 'حفظ التعديلات' : 'حفظ الحصة'}
                    </Button>
                  </Box>
                </Grid>
              </Grid>
